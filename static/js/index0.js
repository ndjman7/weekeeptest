    ...
    this.$el.on('click', '.time-slot', function () {
      ...
      if (!plugin.isSelecting()) {  // if we are not in selecting mode
        ...
        if (day == plugin.$selectingStart.data('day')) {  // if clicking on the same day column
          ...
          var start = plugin.$selectingStart.data('time');
          var end = $(this).data('time');
          (function() {
            var isDim = true;   //dimmed 레이어를 감지하기 위한 boolean 변수
            isDim ? $('.dim-layer').fadeIn() : plugin.$el.fadeIn();
            if (day == '1') {
              $('.wkday').text('일');
            } else if (day == '2') {
              $('.wkday').text('월');
    ...
    $.each(schedule, function(d, ds) {
      ...
      $.each(ds, function(_, s) {
        ...
        for (i = 0; i < $slots.length; i++) {
          if ($slots.eq(i).data('time') == s[0]) { $slots.eq(i).text(s[2]) }
